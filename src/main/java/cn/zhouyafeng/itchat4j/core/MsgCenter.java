...
public class MsgCenter {
  ...
  public static JSONArray produceMsg(JSONArray msgList) {
    ...
    for (int i = 0; i < msgList.size(); i++) {
      ...
      if (fromUserName.contains("@@") || m.getString("ToUserName").contains("@@")) { // 群聊消息
      ...
      if (fromUserName.equalsIgnoreCase(core.getUserName())) {
        fromUser = core.getUserSelf();
      } else {
        fromUser = core.getUserInfoMap().get(fromUserName);
        if (fromUser == null) {
          fromUser = core.getGroupPropertyMap().get(fromUserName);
        }
      }
      ...
      if (fromUser != null) {
        ...
        if (StringUtils.isBlank(nickName)) {
          nickName = fromUser.getString("RemarkName");
        }
      }
      if (nickName == null) {
        nickName = fromUserName;
      }
      LOG.info("收到消息一条，来自: " + nickName);
      result.add(m);
    }
    return result;
  }
